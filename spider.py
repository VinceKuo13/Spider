...
首次运行会自动读取同目录 beijing_ggzy_config.yaml；如果没有，则使用 DEFAULT_CONFIG。
"""
...
CONFIG_PATH = BASE_DIR / "beijing_ggzy_config.yaml"
...
def load_config(path: Path = CONFIG_PATH) -> Dict[str, Any]:
    ...
    if path.exists():
        if yaml is None:
            raise RuntimeError("检测到 beijing_ggzy_config.yaml，但当前环境没有 pyyaml，请执行：pip install pyyaml")
