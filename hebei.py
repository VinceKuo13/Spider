# -*- coding: utf-8 -*-
"""
河北政府采购网爬虫：采购意向 + 招标公告 + 中标公告

适用网站：
1) 采购意向：https://www.ccgp-hebei.gov.cn/province/zfcgyxgg/zfcgyx/
2) 招标公告：https://www.ccgp-hebei.gov.cn/province/cggg/zbgg/
3) 中标公告：https://www.ccgp-hebei.gov.cn/province/cggg/zhbgg/

核心特点：
1. 不使用 Selenium，直接请求 was5/web/search 接口。
2. 搜索范围固定为标题，即接口参数 doctitle=关键词。
3. 支持 2025-01-01 至今天的数据，自动按不超过 365 天切分时间段。
4. 同时抓取采购意向 zfcgyx、招标公告 zbgg 和中标公告 zhbgg，并修复 was5 JSON 字段 docPubUrl/docTitle 的解析。
...
ANN_TYPES = {
    "采购意向": {
        "lanmu": "zfcgyx",
        "referer": "https://www.ccgp-hebei.gov.cn/province/zfcgyxgg/zfcgyx/",
    },
    "招标公告": {
        "lanmu": "zbgg",
...
def apply_yaml_config() -> None:
    """
    读取项目根目录 config.yaml。
    只覆盖关键词、开始日期、结束日期、输出目录等通用配置；
    各省网站接口、栏目编码等仍写在本文件内。
    """
    global KEYWORDS, START_DATE, END_DATE
    global OUTPUT_DIR, RAW_DIR, JSON_PATH, EXCEL_PATH, PROGRESS_PATH, LOG_PATH
    ...
    try:
        ...
        print(f"[WARN] 读取 config.yaml 失败，使用脚本默认配置：{e}")
        return
    ...
    if cfg.get("start_date"):
        START_DATE = str(cfg.get("start_date")).strip()

    if cfg.get("end_date") is not None:
        END_DATE = str(cfg.get("end_date") or "").strip()

    hebei_cfg = cfg.get("hebei") or {}
    out_dir = hebei_cfg.get("output_dir") or cfg.get("output_dir")
    if out_dir:
...
def normalize_detail_url(url: str) -> str:
    """规范化详情 URL，避免相对路径、查询参数顺序、无意义参数导致重复。"""
    if not url:
        return ""
    url = urljoin(BASE_URL + "/", str(url).strip())
    try:
        p = urlparse(url)
        drop_keys = {"t", "_", "_t", "timestamp", "stmp", "random", "rand"}
        pairs = [(k, v) for k, v in parse_qsl(p.query, keep_blank_values=True) if k not in drop_keys]
        pairs.sort()
...
def normalize_title_key(s: Any) -> str:
    s = clean_text(s)
    s = re.sub(r"[\s　]+", "", s)
    s = re.sub(r"[，。；;：:、,.!?！？（）()【】\[\]《》<>\"'“”‘’/\\|_-]+", "", s)
    return s
...
def record_dedup_key(r: Dict[str, Any]) -> Tuple[str, str, str, str]:
    """导出层去重。采购意向优先按标题+日期+项目名去重，其余优先按 URL 去重。"""
    ann = clean_text(r.get("公告类型") or r.get("announcement_type") or "")
    title = normalize_title_key(r.get("标题") or r.get("title") or "")
    pub = normalize_date_text(r.get("发布时间") or r.get("publish_date") or "")
    project = normalize_title_key(r.get("项目名称") or "")
    url = normalize_detail_url(r.get("详情链接") or r.get("detail_url") or "")
    if ann == "采购意向" and title:
        return (ann, title, pub, project)
    if url:
        return ("URL", url, "", "")
    return (ann, title, pub, project)
...
def recursive_find_records(obj: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(obj, dict):
        ...
        # 河北 was5 接口采购意向返回字段通常是 docPubUrl + docTitle。
        # 原代码没有识别 docPubUrl，导致 JSON 解析失败，退回正则扫描原始 JSON，
        # 从而出现采购意向被重复/异常解析的问题。
        url_keys = {"url", "href", "link", "docurl", "docpuburl", "docurl2"}
        title_keys = {"title", "doctitle", "name", "docname"}
        if keys.intersection(url_keys) and keys.intersection(title_keys):
            records.append(obj)
        for v in obj.values():
...
def item_from_json_record(d: Dict[str, Any], ann_type: str, keyword: str) -> Optional[Dict[str, Any]]:
    ...
    title = first_key("title", "doctitle", "docTitle", "name")
    href = first_key("url", "href", "link", "docurl", "docPubUrl", "docpuburl")
    if not href or ".html" not in href:
        return None
    url = normalize_detail_url(urljoin(BASE_URL + "/", href))
    ...
    return {
        ...
        "publish_date": pub,
        "detail_url": url,
        "list_region": first_key("district", "region", "area", "ptName", "deptName", "地区"),
        "list_purchaser": first_key("purchaser", "purchasername", "unitName", "agencyFullName", "采购人"),
        "list_raw_text": "",
    }
...
def parse_list_items(response_text: str, ann_type: str, keyword: str) -> Tuple[List[Dict[str, Any]], Optional[int]]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not href or ".html" not in href:
            continue
        # 河北政府采购网不同栏目详情路径不同：
        # /cggg/zbgg/ 招标公告；/cggg/zhbgg/ 中标公告；/zfcgyxgg/zfcgyx/ 采购意向。
        if (
            "/cggg/" not in href
            and "/zfcgyxgg/" not in href
            and "zbgg" not in href
            and "zhbgg" not in href
            and "zfcgyx" not in href
        ):
            continue
    ...
    if not candidates:
        ...
        for pat in url_patterns:
            for m in re.finditer(pat, html):
                href = m.group(0)
                url = normalize_detail_url(urljoin(BASE_URL + "/province/cggg/", href))
                if url in seen_urls or ("/cggg/" not in url and "/zfcgyxgg/" not in url and "zfcgyx" not in url):
                    continue
                seen_urls.add(url)
                # 在链接附近取标题
...
def dedup_list_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        if item.get("detail_url"):
            item["detail_url"] = normalize_detail_url(item.get("detail_url", ""))
        key = (
            item.get("detail_url")
            or (
                item.get("announcement_type"),
                normalize_title_key(item.get("title", "")),
                normalize_date_text(item.get("publish_date", "")),
            )
        )
        if key in seen:
            continue
...
def parse_detail_page(html: str, url: str, ann_type: str) -> Dict[str, Any]:
    ...
    parsed: Dict[str, Any] = {
        ...
        "项目名称": extract_simple_field(text, ["项目名称", "采购项目名称"], ["预算金额", "采购需求", "预计采购时间", "三、", "中标", "成交"]),
        ...
        "预算金额": extract_simple_field(text, ["预算金额", "预算金额（万元）", "预算金额(万元)"], ["最高限价", "采购需求", "预计采购时间", "合同履行期限"]),
        "最高限价": extract_simple_field(text, ["最高限价"], ["采购需求", "合同履行期限", "二、"]),
        "采购需求": extract_simple_field(text, ["采购需求", "采购需求概况"], ["预计采购时间", "本项目", "二、申请人", "备注"]),
        "采购方式": extract_simple_field(text, ["采购方式"], ["采购单位", "采购人", "代理机构", "预算金额"]),
        "预计采购时间": extract_simple_field(text, ["预计采购时间"], ["备注", "本次公开", "附件"]),
        "中标/成交供应商": "",
        "中标/成交金额": "",
        "标的信息": "",
        "详情链接": url,
    ...
    # 采购意向很多字段在表格中，做一个表格兜底。
    if ann_type == "采购意向":
        for row in parse_all_tables(soup):
            if not parsed.get("项目名称"):
                parsed["项目名称"] = row_get(row, ["采购项目名称"]) or row_get(row, ["项目名称"])
            if not parsed.get("预算金额"):
                parsed["预算金额"] = row_get(row, ["预算金额"])
            if not parsed.get("采购需求"):
                parsed["采购需求"] = row_get(row, ["采购需求"]) or row_get(row, ["采购需求概况"])
            if not parsed.get("预计采购时间"):
                parsed["预计采购时间"] = row_get(row, ["预计采购时间"])
            if parsed.get("项目名称") or parsed.get("预算金额"):
                break
...
def normalize_records_for_export(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    # 去重：规范化 URL；采购意向按 标题+日期+项目名 兜底去重。
    seen = set()
    out = []
    for r in records:
        if r.get("详情链接"):
            r["详情链接"] = normalize_detail_url(r.get("详情链接", ""))
        if r.get("detail_url"):
            r["detail_url"] = normalize_detail_url(r.get("detail_url", ""))
        key = record_dedup_key(r)
        if key in seen:
            continue
        seen.add(key)
        out.append(r)
...
def crawl_one(
    ...
    for page in range(1, MAX_PAGES_PER_WINDOW + 1):
        ...
        page_urls = {normalize_detail_url(x.get("detail_url", "")) for x in items if x.get("detail_url")}
...
def main() -> None:
    ...
    logging.info("开始爬取：%s；栏目=采购意向/招标公告/中标公告；搜索范围=标题 doctitle；关键词=%s", SITE_NAME, KEYWORDS)
    logging.info("总时间范围：%s 至 %s；自动切分：%s", START_DATE, end_date, windows)

    records = load_json_list(JSON_PATH)
    progress = load_progress()
    visited_urls = {normalize_detail_url(x) for x in progress.get("visited_urls", []) if x}
    # 如果已有 JSON，但 progress 丢失，也从 JSON 里恢复
    for r in records:
        if r.get("详情链接"):
            visited_urls.add(normalize_detail_url(r["详情链接"]))
