...
2. 列表接口为 http://www.ccgp-shandong.gov.cn:8087/api/website/site/getListByCode
3. 采购意向 colCode=2500
...
LIST_API = "http://www.ccgp-shandong.gov.cn:8087/api/website/site/getListByCode"
...
def try_auto_get_captcha(session: requests.Session) -> Tuple[str, Optional[Path]]:
    ...
    通过山东站真实验证码接口获取验证码。
    用户提供的真实接口：
        http://www.ccgp-shandong.gov.cn:8087/api/website/captcha

    正常返回 JSON，里面包含 captchaUuid 和验证码图片/base64/图片URL。
    """
    candidates = [
        "http://www.ccgp-shandong.gov.cn:8087/api/website/captcha",
        f"{BASE_URL}:8087/api/website/captcha",
    ]
